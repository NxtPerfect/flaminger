...
export default function ActionButton({ type = "normal", children = "Test", isLoading = false }: Props) {
  ...
  switch (type) {
    case "big":
      button = <button className={`flex mt-8 bg-orange-600 hover:bg-red-600 hover:animate-none text-white rounded-md p-8 py-2 text-[2rem] leading-tight animate-pulse duration-500 {isLoading ? "bg-neutral-200" : ""}`} disabled={isLoading}>{children}</button>
      break;
    case "alt":
      button = <button className={`flex mt-8 text-orange-600 bg-neutral-900 rounded-md p-8 py-2 text-[2rem] leading-tight animate-pulse hover:bg-neutral-800 hover:animate-none hover:ease-in-out duration-500 {isLoading ? "bg-neutral-200 cursor-wait" : ""}`} disabled={isLoading}>{children}</button>
      break;
    case "formSubmit":
