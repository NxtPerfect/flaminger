...
export default function ActionButton({ type, children, isLoading }: Props) {
  ...
  switch (type) {
    case "big":
      button = <button className="flex mt-8 bg-neutral-400 text-white rounded-md p-8 py-2 text-[2rem] leading-tight animate-pulse cursor-not-allowed duration-500" disabled={isLoading}>{children}</button>
      break;
    case "alt":
      button = <button className="flex mt-8 text-orange-600 bg-neutral-900 rounded-md p-8 py-2 text-[2rem] leading-tight animate-pulse hover:bg-neutral-800 hover:ease-in-out duration-500" disabled={isLoading}>{children}</button>
      break;
    case "formSubmit":
